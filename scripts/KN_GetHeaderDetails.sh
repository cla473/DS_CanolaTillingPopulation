...
#SBATCH --job-name=:KN_GetHeaderDetails
...
INDIR="/OSM/CBR/AF_DATASCHOOL/input/2018-05-03_canola/NB501086_0095_CHelliwell_CSIRO_Brapa_gDNA/*.gz"


#add a header row to our header.txt file
echo "id, filename, barcode, laneNo, index" > headers.txt
...
do
    ...
    #echo $HEADER | cut -d ':' -f 3,4,10 --output-delimiter=', ' >> headers.txt

    #what I want to do is output a line
    #1, Pool25_S1_R1_001.fastq.gz, H72NFAFXX, 1, CGATGT
    #echo $count", "  $(basename "$file")", " ;
     #  $HEADER | cut -d ':' -f 3,4,10 --output-delimiter=', ' >> headers.txt
